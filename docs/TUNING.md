...
Inside the tolerance band the bar fills; outside it drains. Per skill level
(Apprentice/Graduate/Professor, chosen at each quest start):
...
| `ALIGN_RAMP` | `0.5` | extra drift speed gained by a full bar (1.5x at the top). |
| `ALIGN_TOL_SHRINK` | `0.15` | fraction of the tolerance band lost by a full bar. |
...
is persistence + the progress ramp, not raw speed alone. Balance check: the
bar climbs only while time-aligned exceeds drain/(fill+drain) — keep that
break-even duty under ~40% or progress stalls mid-bar (bench-found at 56%).
